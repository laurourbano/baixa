        ...
        grid.innerHTML = INITIAL_CARDS
            ...
            .map(card => {
                ...
                <div class="card border-${color}" data-id="${card.id}" data-color="${color}" draggable="true" ${!isLink ? `onclick="MainApp.copy(this.querySelector('.content-display'), '${card.id}')"` : ''}>
                    ...
                    ` : `
                        <div class="content-display">${card.showDate !== false ? `<span class="date-highlight">${formattedDate}</span> - ` : ''}${card.content.replace(/\[\s*00\/00\/0000\s*\]/g, `<span class="date-highlight">${formattedDate}</span>`)}</div>
